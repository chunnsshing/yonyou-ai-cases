...
assets/logo.svg            Yonyou logo（待 ChunShing 放入官方文件；没有时显示 LOGO 占位）
...
5. **不编造**：资料里没有的数字、客户名、效果指标、"已上线"等事实一律不写；缺必填内容时列出来问用户，而不是自己补。
6. **保密**：这是公开仓库。客户名、内部报价、合同、联系人等敏感信息不写进 YAML；原始资料放 `raw/`（不会上传）。
7. 写完运行校验，修到 0 错误 0 警告：
   ```bash
   pip install pyyaml   # 首次
   python build.py --strict
   ```
   然后打开 `dist/index.html` 看一眼，确认版面没有溢出。
8. 给用户一个简短汇总：新增/修改了哪些场景、各自状态、哪些信息需要他确认。
...
- **换 logo**：把官方文件放到 `assets/`，并在 `site.yaml` 的 `logo` 改成对应文件名（svg/png 都行）。
- **导出 PDF**：在页面上点"导出本领域 PDF"（首页）或"导出 PDF"（场景页），浏览器打印选"另存为 PDF"，每个场景一页横版。
- **快捷键**：场景页 ← → 切换同领域场景，Esc 返回领域列表。
